def getInitialPopulation_continue(n_population,n_gene,SearchRegion):
    ...
    for i in range(n_population):
        while np.isinf(population[i,-1]) or np.isnan(population[i,-1]):
            population[i,:n_gene] = (np.log10(X0*10**(np.random.rand(len(X0))*2.-1.)) - SearchRegion[0,:])/(SearchRegion[1,:] - SearchRegion[0,:])

...
def ga_v1(nth_paramset,max_generation,n_population,n_children,n_gene,
    ...
    while generation < max_generation:
        ...
        print('Generation%d: Best Fitness = %e'%(generation+1,population[0,-1]))
        best_indiv = decode_gene2variable(population[0,:n_gene],search_region)

        if population[0,-1] < best_fitness:
            np.save('./out/%d/generation.npy'%(nth_paramset),generation+1)
            np.save('./out/%d/fit_param%d.npy'%(nth_paramset,generation+1),best_indiv)
        best_fitness = population[0,-1]
        np.save('./out/%d/best_fitness.npy'%(nth_paramset),best_fitness)
        
        np.save('./out/%d/count_num.npy'%(nth_paramset),generation+1)
